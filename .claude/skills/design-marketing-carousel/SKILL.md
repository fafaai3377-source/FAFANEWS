...
> - og:image가 풍부한 매체 우선: TechCrunch, The Verge, Figma Blog, Fast Company, Dezeen 등
> - roundup/digest URL(여러 소식 모은 페이지) 사용 금지 — og:image가 무관한 이미지로 나옴
> - 같은 URL을 두 카드에 쓰는 것 절대 금지

> ⛔ **죽은 링크/404 절대 금지 (사용자 핵심 요구).**
> - 카드에 넣기 전 모든 URL이 **실제로 열리는지** 직접 확인(열어서 404·삭제·리다이렉트면 폐기).
> - `render_carousel.py`의 `validate_url()`이 렌더 시점에 한 번 더 검증해, 죽었으면 도메인 루트로
>   자동 폴백하지만 — **수집 단계에서 살아있는 정확한 기사 URL을 넣는 것이 원칙.**
> - 이미지는 무조건 **기사 내용과 직접 관련된 것**만. og:image가 무관하면 그 URL을 버리고
>   더 나은 전용 기사 URL로 교체한다(스스로 판단해 교체).
