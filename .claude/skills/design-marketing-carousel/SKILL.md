...
**수집 기간은 요일에 따라 다르다 — 반드시 확인 후 검색.**

| 오늘 요일 | 수집 대상 기간 | 검색 시 포함할 날짜 |
|-----------|--------------|-------------------|
| 화~금 | 전날 ~ 오늘 (24시간) | 어제·오늘 날짜 모두 |
| 월요일 | 금요일 ~ 오늘 (72시간) | 금·토·일·오늘 날짜 |
| 토·일 (수동 실행 시) | 전날 ~ 오늘 (24시간) | 어제·오늘 날짜 |

> ⚠️ **오래된 자료 절대 금지.** 검색 시 날짜 필터(`YYYY-MM-DD`, `after:YYYY-MM-DD`)를
> 반드시 포함해서 수집 기간 내 발행된 콘텐츠만 가져온다.
> 서핏·이오플래닛은 최신 글이 상단에 있으므로 오늘 날짜 기준 확인 필수.
...
WebSearch로 **수집 기간 날짜를 검색어에 포함**하여 검색:
- **서핏(surfit.io)** — AI·디자인·마케팅 트렌드 큐레이션
  - 검색어: `surfit.io AI 디자인 마케팅 YYYY-MM-DD` (수집기간 날짜 각각 검색)
- **이오플래닛(eopla.net)** — UX·프로덕트·브랜드 전문 뉴스레터
  - 검색어: `site:eopla.net YYYY-MM-DD` (수집기간 날짜 각각 검색)
- **브런치(brunch.co.kr)** — 전문가 실무 인사이트·케이스스터디
  - 검색어: `site:brunch.co.kr 디자인 AI 마케팅 YYYY년 MM월` (최신순)
- **요즘IT(yozm.wishket.com)** — 개발·디자인·마케팅 트렌드 아티클
- **폴인(folin.co)** — 브랜드·마케팅·조직문화 심층 리포트
- **모비인사이드(mobiinside.co.kr)** — 마케팅·스타트업 실무 인사이트

월요일 검색 예시 (오늘이 2026-06-15 월요일인 경우):
```
surfit.io AI 디자인 2026-06-13 OR 2026-06-14 OR 2026-06-15
site:eopla.net after:2026-06-12
```
